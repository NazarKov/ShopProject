...
namespace ShopProject.ViewModel.AdminPage.UserManagement.User
{
    internal class UsersDataViewModel : ViewModel<UsersDataViewModel>, IViewModelLoadResourse
    {
        ...
        public UsersDataViewModel(IUserService userService, IMessageBoxControlService messageBoxControlService)
        {
        ...
        private void UpdateUser(object parameter)
        {
            ...
            if (user != null)
            {
                ...
                {
                    _messageBoxControlService.Show("Ви не обрали елемент.", "Warninng", MessageBoxType.Warning, "UserManagementSnadow");
                }
            }
        }

        public ICommand SetUserStatusDisabledCommand { get => CreateCommandParameterAsync<object>(SetUserStatusDisabled); }
        private async Task SetUserStatusDisabled(object parameter)
        {
            var user = parameter as IList;
            if (user != null)
            {
                if (user.Count == 1)
                {
                    if (await _messageBoxControlService.Show("Вимкнути?.", "Informations", MessageBoxType.Question, "UserManagementSnadow"))
                    {
                        var result = await _userService.UpdateParameter("Status", TypeStatusUser.ProfileIsDisabled, ((UserModel)user[0]).ToUser());
                        if (result.IsSuccess)
                        {
                            await SetFieldPage();
                            await _messageBoxControlService.Show("Користувача вимкнено.", "Informations", MessageBoxType.Success, "UserManagementSnadow");
                        }
                        else
                        {
                            await _messageBoxControlService.Show("Невдалося виконати операцію.", "Error", MessageBoxType.Error, "UserManagementSnadow");
                        }
                    }
                }
                else
                {
                    await _messageBoxControlService.Show("Ви не обрали елемент.", "Warninng", MessageBoxType.Warning, "UserManagementSnadow");
                }
            }
        }

        public ICommand SetUserStatusEnabledCommand { get => CreateCommandParameterAsync<object>(SetUserStatusEnabled); }
        private async Task SetUserStatusEnabled(object parameter)
        {
            var user = parameter as IList;
            if (user != null)
            {
                if (user.Count == 1)
                {
                    if (await _messageBoxControlService.Show("Увімкнути?.", "Informations", MessageBoxType.Question, "UserManagementSnadow"))
                    {
                        var result = await _userService.UpdateParameter("Status", TypeStatusUser.Unknown, ((UserModel)user[0]).ToUser());
                        if (result.IsSuccess)
                        {
                            await SetFieldPage();
                            await _messageBoxControlService.Show("Користувача увімкнено.", "Informations", MessageBoxType.Success, "UserManagementSnadow");
                        }
                        else
                        {
                            await _messageBoxControlService.Show("Невдалося виконати операцію.", "Error", MessageBoxType.Error, "UserManagementSnadow");
